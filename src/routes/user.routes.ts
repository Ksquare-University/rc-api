...
// CREATE - [POST]
// Allow a customer to sign up to our system
...
// Only allow admins and superadmins to create owners (rest owners)
UserRouter.post("/owner", async (req: Request, res: Response) => {
  const { displayName, email, password } = req.body;
  ...
  try {
    const newOwnerId = await createUser(displayName, email, password, "owner");
...
UserRouter.get("/:userId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(400)
      .send({ error: "Couldn't read user. The requested route doesn't exist" });
  }
});
