...
export const initDb = () => {
  ...
  const createUser = (
    ...
    User.create({ id, role, user_name, email, password });
  };
  const createOwner = (full_name: string, phone: number, user_id: string) => {
    Owner.create({ full_name, phone, user_id });
  };
  const createClient = (
    full_name: string,
    phone_number: number,
    ...
    Customer.create({ full_name, phone_number, user_id });
  };
  const createCourier = (
    full_name: string,
    phone_number: number,
    ...
    Courier.create({ full_name, phone_number, user_id });
  };
  type hour = string;
  ...
  const createManager = (user_id: string, restaurant_id: number) => {
    Manager.create({ user_id, restaurant_id });
  };
  const createOpening = (
    ...
    OpeningDays.create({ restaurant_id, day, opening_hour, closing_hour });
  };
  const createItems = (name: string, price: number, restaurant_id: number) => {
    Item.create({ name, price, restaurant_id });
  };

  const createOrderStatus = (name: string) => {
    OrderStatus.create({ name });
  };
  const createOrder = (
    client_id: number,
    courier_id: number,
    ...
    Order.create({ client_id, courier_id, restaurant_id });
  };
  const createOrderItems = (
    ...
    OrderItems.create({ order_id, item_id, quantity });
  };
  ...
  const createClientAddress = (
    ...
    ClientAddress.create({ client_id, address, reference, zip_code, city_id });
  };
  ...
  const createPaymentMethod = (method: string, billing_details_id: number) => {
    PaymentMethod.create({ method, billing_details_id });
  };
  ...
  //I take the data from || "customer" | "courier" | "owner" | "admin" | "superadmin" | "manager";
  const passwordSuperAdmin = 'root123';
  const password = 'test123';
  createUser(
    'SVPKICCqw4gQunZkh9RGQYlYqTa2',
    'owner',
    'JonnyBravo',
    'jonathan@owner.com',
    password
  ...
  createUser(
    'AIPCkVBKTZdGBIhiIrlJTGXnpim1',
    'superadmin',
    'Superadmin',
    'rappi@superadmin.com',
    passwordSuperAdmin
  );

  createUser(
    'DXCISM9Dqbcr814VjXynl6w7CYC3',
    'customer',
    'Palomitas',
    'brandon@customer.com',
    password
  );
  createUser(
    'lHhRx3Pw4Seu5cExtWnldaSjRhM2',
    'customer',
    'Genaro123',
    'genaro@customer.com',
    password
  ...
  createUser(
    'RoPXOPVJnoZcdYbdHpjDBmq31ym2',
    'customer',
    'Rodrigo12',
    'rodrigo@customer.com',
    password
  );
  createUser(
    '2sPhEfPCyDPLIChpQPfbeFgf6s32',
    'customer',
    'IamDannae',
    'dannae@customer.com',
    password
  ...
  createCourier('Jhon Courier', 1546855251, 'CQKoMpHPQDRXyI53gkv2pM7FAQW2');
  ...
  createOrderStatus('Accepted');
  createOrderStatus('Cooking');
  createOrderStatus('On route');
  createOrderStatus('Delivered');
  createOrderStatus('Canceled');
  ...
  createClientAddress(
    2,
    'Street 31-1 No 333 38b and 30b',
    'Green house with and tree in the door',
    97206,
    1
  );
  createClientAddress(
    3,
    'Street 43 Chuburna-In',
    'In fron of Inbursa bank',
    97206,
    1
  );
  createClientAddress(
